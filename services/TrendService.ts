...
/**
 * Legace class-based TrendService for Supabase integration.
 * Updated to use the new UserTrendState logic.
 */
export class TrendService {
  static async getUserTrend(userId: string, createdAt?: string): Promise<UserTrendState> {
    try {
      const now = new Date();
      ...
      return calculateUserTrend(activities);
    } catch (error) {
      console.error('Error fetching trend from Supabase:', error);
      return 'NEUTRAL';
    }
  }
}
